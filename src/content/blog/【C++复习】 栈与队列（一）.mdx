...
我们用 栈名.函数名() 的方式调用函数，例如`s.push(1);`代表向名称为s的栈中，入栈了元素1。
...
栈是一种先进后出的数据结构，它只能在一端进行插入和删除操作。在C++中，我们可以使用`数组`或`STL中的stack模板类`来模拟栈的操作。栈的常用操作包括**入栈、出栈、取栈顶元素、判断栈是否为空、清空栈和获取栈内元素个数**。
