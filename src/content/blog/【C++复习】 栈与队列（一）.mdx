...
先定义全局数组st作为栈的存储结构，并将栈顶指针TOP初始化为0。接下来定义相关操作函数：
...
有了这些自定义函数之后，就可以调用这些函数模拟栈的操作，比如尝试将整数1、2、3、4、5入栈，接着输出此时的栈顶，最后输出栈内所有的元素。
...
`标准模板库（STL）`是一些常用数据结构（栈、队列等）和算法（sort、swap等）的模板的集合。有了STL，不必再写太多的标准数据结构和算法，并且可获得很高的性能。

除了自己实现栈之外，C++标准模板库（STL）也提供了栈的模板类，可以直接使用，非常方便。
...
**empty()**:检测栈是否为空，返回true为空，false为非空

**size()**:返回栈中元素个数函数的使用

我们用 栈名.函数名() 的方式调用函数，例如s.push(1);代表向名称为s的栈中，入栈了元素1。
...
栈是一种先进后出的数据结构，它只能在一端进行插入和删除操作。在C++中，我们可以使用数组或STL中的stack模板类来模拟栈的操作。栈的常用操作包括入栈、出栈、取栈顶元素、判断栈是否为空、清空栈和获取栈内元素个数。
